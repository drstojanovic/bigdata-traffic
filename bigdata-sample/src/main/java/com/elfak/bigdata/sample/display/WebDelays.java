...
public class WebDelays {
    ...
    public static void main(String[] args) throws Exception {
        ...
        options.setOutboundPermitted(Collections.singletonList(new PermittedOptions().setAddress("delays")));
        ...
        System.out.println("consume from Kafka Topic " + uberTopic + " publish to eventbus delays");
       
        while (true) {
            ConsumerRecords<String, String> records = consumer.poll(0);
            for (ConsumerRecord<String, String> record : records) {
                vertx.eventBus().publish("delays", record.value());
